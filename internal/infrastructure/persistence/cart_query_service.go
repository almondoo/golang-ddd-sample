...
// コンテキスト所有）を SQL レベルで直接 JOIN する。catalog のドメイン
// ロジック（Product 等）を呼び出しているわけではないが、products の
// 物理スキーマ（テーブル名・カラム名）への結合は読み取り側に残っている。
// そのため catalog 側でテーブル名やカラム名を変更した場合、コンパイル
// エラーという形では検知できず、実行時にこのクエリが壊れうる
// （コンテキスト間の結合が完全になくなっているわけではない点に注意）。
