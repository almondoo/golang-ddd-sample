...
func NewRegisterProductUseCase(repo catalog.Repository, txManager tx.Manager) *RegisterProductUseCase {
...
func (uc *RegisterProductUseCase) Execute(ctx context.Context, input RegisterProductInput) (RegisterProductOutput, error) {
	...
	product, err := catalog.NewProduct(input.Name, input.Description, price)
	if err != nil {
		return RegisterProductOutput{}, err
	}
